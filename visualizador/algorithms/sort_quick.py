...
def init(vals: list[int]) -> None:
    """
    Inicializa el estado global del algoritmo Quick Sort.

    :param vals: La lista inicial de enteros a ordenar.
    """
...
def _swap(a: int, b: int) -> None:
    """
    Realiza el intercambio de items[a] e items[b].
    """
    global items
...
def step() -> dict:
    """
    Ejecuta un único micro-paso del Quick Sort.

    :return: Diccionario con el estado del paso: {"a": int, "b": int, "swap": bool, "done": bool}
    """
    global items, n, stack, low, high, pivot_index, i, j, partición_terminada
